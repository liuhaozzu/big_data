...
import com.liuhaozzu.bigdata.hadoophbase.common.DateEnum;
...
import com.liuhaozzu.bigdata.hadoophbase.transformer.model.dim.base.DateDimension;
...
import com.liuhaozzu.bigdata.hadoophbase.transformer.mr.TransformerOutputFormat;
import com.liuhaozzu.bigdata.hadoophbase.util.JdbcManager;
...
public class NewInstallUserRunner implements Tool {
	...
	private void calculateTotalUsers(Configuration conf) {
		...
		try {
			...
			// 获取昨天的date dimension
			DateDimension yesterdayDimension = DateDimension.buildDate(date - GlobalConstants.DAY_OF_MILLISECONDS,
					DateEnum.DAY);
			...
			// 更新操作
			pstmt = conn.prepareStatement(
					"INSERT INTO `stats_device_browser`(`platform_dimension_id`,`browser_dimension_id`,`date_dimension_id`,`total_install_users`) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE `total_install_users` = ?");
			for (Map.Entry<String, Integer> entry : oldValueMap.entrySet()) {
