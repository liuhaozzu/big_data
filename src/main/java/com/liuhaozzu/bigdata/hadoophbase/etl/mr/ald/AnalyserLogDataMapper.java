package com.liuhaozzu.bigdata.hadoophbase.etl.mr.ald;
...
import com.liuhaozzu.bigdata.hadoophbase.etl.util.LoggerUtil;
...
public class AnalyserLogDataMapper extends Mapper<LongWritable, Text, NullWritable, Put> {
	 ...
	 * @throws IOException
	 */
	private void handleData(Map<String, String> clientInfo, EventEnum event, Context context)
		...
		if (StringUtils.isNotBlank(serverTime)) {
			// 要求服务器时间不为空
			clientInfo.remove(EventLogConstants.LOG_COLUMN_NAME_USER_AGENT); // 浏览器信息去掉,已经封装成对象了
