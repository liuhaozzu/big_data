package com.liuhaozzu.bigdata.hadoophbase.etl.util;

import com.liuhaozzu.bigdata.hadoophbase.common.GlobalConstants;
import com.liuhaozzu.bigdata.hadoophbase.etl.util.ip.IPSeeker;
				...
				if (index > 0) {
					...
					if (index == length - 1) {
						...
						if (index2 > 0) {
							...
							info.setCity(country.substring(index + 1, Math.min(index2 + 1, length))); // 设置city
						}
					}
				} else {
					...
					case "内蒙":
						...
						if (country != null && !country.isEmpty()) {
							...
							if (index > 0) {
								info.setCity(country.substring(0, Math.min(index + 1, country.length()))); // 设置市
							}
						}
						break;
					case "广西":
					...
					case "重庆":
						...
						if (country != null && !country.isEmpty()) {
							...
							if (index > 0) {
								...
								if (ch != '校' || ch != '小') {
									info.setCity(country.substring(0, Math.min(index + 1, country.length()))); // 设置区
								}
							}
