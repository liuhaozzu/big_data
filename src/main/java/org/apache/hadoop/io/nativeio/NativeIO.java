...
public class NativeIO {
	...
	public static class Windows {
		 ...
		 * maximum working set size to the new values. Controlling the working
		 * set size of the process also controls the amount of memory it can
		 * lock.
		 *
		 * @param delta
	 ...
	 *
	 * In POSIX platform: It uses FileChannel#transferTo() which internally
	 * attempts unbuffered IO on OS with native sendfile64() support and falls
	 * back to buffered IO otherwise.
	 *
	 * It minimizes the number of FileChannel#transferTo call by passing the the
	 * src file size directly instead of a smaller size as the 3rd parameter.
	 * This saves the number of sendfile64() system call when native
	 * sendfile64() is supported. In the two fall back cases where sendfile is
	 * not supported, FileChannle#transferTo already has its own batching of
	 * size 8 MB and 8 KB, respectively.
	 *
	 * In Windows Platform: It uses its own native wrapper of CopyFileEx with
	 * COPY_FILE_NO_BUFFERING flag, which is supported on Windows Server 2008
	 * and above.
	 *
	 * Ideally, we should use FileChannel#transferTo() across both POSIX and
	 * Windows platform. Unfortunately, the
	 * wrapper(Java_sun_nio_ch_FileChannelImpl_transferTo0) used by
	 * FileChannel#transferTo for unbuffered IO is not implemented on Windows.
	 * Based on OpenJDK 6/7/8 source code,
	 * Java_sun_nio_ch_FileChannelImpl_transferTo0 on Windows simply returns
	 * IOS_UNSUPPORTED.
	 *
	 * Note: This simple native wrapper does minimal parameter checking before
	 * copy and consistency check (e.g., size) after copy. It is recommended to
	 * use wrapper function like the Storage#nativeCopyFileUnbuffered() function
	 * in hadoop-hdfs with pre/post copy checks.
	 *
	 * @param src
	 *            The source path
	 * @param dst
